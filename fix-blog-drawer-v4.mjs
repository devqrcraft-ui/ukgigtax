...
<div class="bdrawer" id="bdrawer">
  ...
  <a href="/blog/delivery-driver-expenses-2026" class="bdrawer-link">Expenses Checklist<span class="arr">&#8594;</span></a>
  ...
  <a href="/blog/gig-worker-pension-uk-2026" class="bdrawer-link">Pension Guide for Gig Workers<span class="arr">&#8594;</span></a>
  <div class="bdrawer-div"></div>
...
for (const file of files) {
  ...
  const hasOldInlineDrawer = content.includes('id="blog-drawer"');
  const hasOldBDDrawer = content.includes('id="bdrawer"');

  if (!hasOldInlineDrawer && !hasOldBDDrawer) {
    skipped++;
    continue;
  }
  ...
  if (hasOldBDDrawer) {
    // Replace from <style> line before bdrawer-overlay to closing </script>
    const styleStart = lines.findIndex(l => l.includes('.bdrawer-overlay{') || l.includes('.bdrawer-overlay {'));
    const scriptEnd = lines.findIndex((l, i) => i > styleStart + 5 && l.includes('closeBD()') && l.includes('</script>'));
    if (styleStart !== -1 && scriptEnd !== -1) {
      const before = lines.slice(0, styleStart - 1); // -1 to include <style> tag
      const after = lines.slice(scriptEnd + 1);
      // find actual <style> tag line
      let styleTagLine = styleStart - 1;
      for (let i = styleStart; i >= Math.max(0, styleStart - 3); i--) {
        if (lines[i].trim() === '<style>') { styleTagLine = i; break; }
      }
      const before2 = lines.slice(0, styleTagLine);
      content = [...before2, newDrawerBlock, ...after].join('\n');
    }
  } else if (hasOldInlineDrawer) {
    // Old inline drawer: find overlay line and drawer end
    ...
    if (overlayLine === -1 || drawerLine === -1) { skipped++; continue; }
    ...
    for (let i = drawerLine; i < Math.min(drawerLine + 100, lines.length); i++) {
      ...
      if (i > drawerLine && depth <= 0) { drawerEndLine = i; break; }
    }
